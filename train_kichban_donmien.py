...
def load_source_array(file_path: Path) -> np.ndarray:
    frame = read_csv_flexible(file_path)
    columns = select_acc_columns(frame, ACC_SENSOR_KEYS, EXPECTED_CHANNELS)
    ...
    for domain in domains:
        ...
        for source_path in tqdm(csv_files, desc=f"domain {domain}", leave=False):
            ...
            if npy_path.exists():
                ...
                continue
            try:
                array = load_source_array(source_path)
...
def plot_combined_confusion_matrices(items: list[dict], save_path: str) -> None:
    if not items:
    ...
    for idx, (ax, item) in enumerate(zip(axes, items)):
        cm = confusion_matrix(item["y_true"], np.argmax(item["y_probs"], axis=1))
        classes = [str(i) for i in range(cm.shape[0])]
        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=classes)
        disp.plot(ax=ax, cmap="Blues", colorbar=False, values_format="d")
...
def class_file(cfg: DataConfig, class_name: str) -> str:
    path = Path(cfg.root) / cfg.domain / f"{class_name}.npy"
    if not path.is_file():
...
def make_loaders(classes: list[str], cfg: DataConfig, batch_size: int, num_workers: int, loader_seed: int):
    files_with_label = [(class_file(cfg, class_name), label) for label, class_name in enumerate(classes)]
    train_index, val_index, test_index = build_index_three_way_per_file(files_with_label, cfg)
    ...
    meta = {
        ...
        "class_files": {class_name: class_file(cfg, class_name) for class_name in classes},
        "window_stride": int(cfg.window),
    }
    return train_loader, val_loader, test_loader, meta


def make_eval_loader(classes: list[str], cfg: DataConfig, batch_size: int, num_workers: int):
    files_with_label = [(class_file(cfg, class_name), label) for label, class_name in enumerate(classes)]
    eval_index = build_index(files_with_label, cfg)
    eval_ds = WindowDataset(eval_index, cfg)
    loader_kwargs = dict(batch_size=batch_size, num_workers=num_workers, pin_memory=torch.cuda.is_available(), worker_init_fn=seed_worker if num_workers > 0 else None)
...
class WaveNetResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        gated = torch.tanh(self.conv_tanh(x)) * torch.sigmoid(self.conv_sig(x))
...
class WaveNetClassifier(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                WaveNetResidualBlock(num_filters, kernel_size, dilation, dropout)
...
def main() -> None:
    ...
    classes = list_classes(dataset_root, TRAIN_VAL_TEST_DOMAIN)
    cfg = DataConfig(
        root=dataset_root,
        domain=TRAIN_VAL_TEST_DOMAIN,
        channels=N_CHANNELS,
        window=WINDOW,
        seed=SPLIT_SEED,
        return_ct=False,
    ...
    train_loader, val_loader, test_loader, meta = make_loaders(classes, cfg, BATCH_SIZE, NUM_WORKERS, GLOBAL_SEED)
    ...
    for extra_domain in EXTRA_TEST_DOMAINS:
        extra_classes = list_classes(dataset_root, extra_domain)
        if extra_classes != classes:
            raise ValueError(f"Class mismatch between {TRAIN_VAL_TEST_DOMAIN} and {extra_domain}: {classes} != {extra_classes}")
        extra_cfg = DataConfig(
            root=dataset_root,
            domain=extra_domain,
            channels=N_CHANNELS,
            window=WINDOW,
        ...
        extra_loader, extra_meta = make_eval_loader(classes, extra_cfg, BATCH_SIZE, NUM_WORKERS)
        extra_loss, extra_acc, extra_true, extra_pred, extra_probs = evaluate_model(model, extra_loader, device, eval_criterion, len(classes))
        extra_scores = build_scores(extra_true, extra_pred)
        extra_test_results[extra_domain] = {
    ...
    history["extra_tests"] = {domain: {"loss": result["loss"], "acc": result["acc"]} for domain, result in extra_test_results.items()}
    run_info = {
        "dataset_name": DATASET_NAME,
        "model_name": MODEL_NAME,
        "scenario_name": SCENARIO_NAME,
        ...
        "extra_tests": extra_test_results,
    ...
    with open(os.path.join(run_dir, f"extra_test_scores_{artifact_prefix}.json"), "w", encoding="utf-8") as handle:
        json.dump(extra_test_results, handle, indent=2)
    plot_performance(history, os.path.join(run_dir, f"learning_curve_{artifact_prefix}"), f"Learning Curve {DATASET_NAME} {MODEL_NAME} {SCENARIO_NAME}")
    plot_combined_confusion_matrices(
        combined_confusion_items,
        os.path.join(run_dir, f"combined_confusion_matrix_{artifact_prefix}_tests.png"),
    )
    print(json.dumps({"run_dir": run_dir, "scores": scores, "extra_tests": extra_test_results}, indent=2))
